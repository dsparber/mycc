...
namespace MyCC.Core.Account.Repositories.Implementations
{
    public class BitcoinCashAccountRepository : AddressAccountRepository
    {
        ...
        protected override Func<string, decimal> Balance => httpContent => JObject.Parse(httpContent)["balance"].ToDecimal() ?? 0;
        protected override decimal BalanceFactor => 1;

        protected override Uri Url => new Uri($"https://cashexplorer.bitcoin.com/insight-api/addr/{Address}");
