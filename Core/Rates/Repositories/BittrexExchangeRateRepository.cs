...
namespace MyCC.Core.Rates.Repositories
{
	public class BittrexExchangeRateRepository : IMultipleRatesRepository
	{
		...
		public async Task<IEnumerable<ExchangeRate>> FetchRates()
		{
			...
			var fetchedRates = (from r in resultJson let market = ((string)r[MarketKey]).Split('-') let rate = 1 / decimal.Parse((string)r[RateKey], NumberStyles.Float, CultureInfo.InvariantCulture) select new ExchangeRate(market[0], true, market[1], true, DateTime.Now, rate) { RepositoryId = TypeId }).ToList();
