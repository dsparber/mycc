	...
	public static class ExchangeRateHelper
	{
		...
		public static async Task UpdateRates(IEnumerable<ExchangeRate> rates, Action<double> progressCallback = null)
		{
			...
			foreach (var r in ratesToUpdate)
			{
				...
				var pref = ApplicationSettings.PreferredBitcoinRepository;
				...
				supportedRepo = supportedRepo != null ? supportedRepo : ExchangeRatesStorage.Instance.Repositories.FirstOrDefault(repo => repo.IsAvailable(r) || repo.IsAvailable(r.Inverse));
