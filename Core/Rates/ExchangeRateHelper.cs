...
namespace MyCC.Core.Rates
{
    public static class ExchangeRateHelper
    {
        ...
        public static ExchangeRate GetRate(Currency.Model.Currency currency1, Currency.Model.Currency currency2)
        {
            ...
            if (currency1.Equals(currency2)) return new ExchangeRate(currency1, currency2, 1);
        ...
        {
            if (currency.Code.Equals("BTC")) return new ExchangeRate(Currency.Model.Currency.Btc, Currency.Model.Currency.Btc, 1);

            if (currency.IsCryptoCurrency) return GetStoredRate(new ExchangeRate(currency, Currency.Model.Currency.Btc));

            var rate = currency.Code.Equals("EUR") ? new ExchangeRate(Currency.Model.Currency.Eur, Currency.Model.Currency.Eur, 1) : GetStoredRate(new ExchangeRate(currency, Currency.Model.Currency.Eur));
            var eurBtc = GetStoredRate(new ExchangeRate(Currency.Model.Currency.Eur, Currency.Model.Currency.Btc));
            return GetCombinedRate(rate, eurBtc);
        }

        private static ExchangeRate GetStoredRate(ExchangeRate neededRate)
        {
            return ExchangeRatesStorage.Instance.StoredRates.ToList().Find(c => c.Equals(neededRate)) ??
                ExchangeRatesStorage.Instance.StoredRates.ToList().Find(c => c.Equals(neededRate.Inverse))?.Inverse;
        }
