...
namespace MyCC.Ui.Android.Data.Get
{
    public class CoinInfoItem
    {
        ...
        public CoinInfoItem(CoinInfoData coinInfoData, string explorer, Context context, Currency currency)
        {
            _data = coinInfoData ?? new CoinInfoData(currency);
            Explorer = explorer;
            _pow = context.Resources.GetString(Resource.String.CoinProofOfWork);
            _pos = context.Resources.GetString(Resource.String.CoinProofOfStake);
            _context = context;
            _currency = currency;
        }
        ...
        public string Hashrate => $"{_data.Hashrate ?? 0:#,0.########} {_context.GetString(Resource.String.GHps)}";
        ...
        public string Blocktime => $"{_data.Blocktime ?? 0:#,0.##} {_context.GetString(Resource.String.UnitSecond)}";
