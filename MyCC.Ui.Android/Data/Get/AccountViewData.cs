...
namespace MyCC.Ui.Android.Data.Get
{
	public class AccountViewData
	{
		...
		public static HeaderDataItem HeaderData(Account account)
		{
			...
			{
				MainText = account.Money.ToStringTwoDigits(ApplicationSettings.RoundMoney),
				InfoText = string.Join(" / ", additionalReferences.Select(m => m.ToStringTwoDigits(ApplicationSettings.RoundMoney)))
			};
		}
