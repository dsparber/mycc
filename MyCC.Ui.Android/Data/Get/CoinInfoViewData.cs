...
namespace MyCC.Ui.Android.Data.Get
{
    public class CoinInfoViewData
    {
        ...
        public CoinInfoItem CoinInfo(Currency currency) => new CoinInfoItem(CoinInfoStorage.Instance.Get(currency), Explorer(currency), _context, currency);
        ...
        public List<ReferenceValueItem> Items(Currency currency)
        {
            return ApplicationSettings.AllReferenceCurrencies.Except(new[] { currency })
                ...
                .OrderByWithDirection(c => SortOrder == SortOrder.Alphabetical ? c.CurrencyCode as object : c.Amount, SortDirection == SortDirection.Ascending)
                .ToList();
        }
        ...
        private void OnSort(SortOrder sortOrder)
        {
            ...
            SortOrder = sortOrder;
            Messaging.UiUpdate.ReferenceTable.Send();
        }
