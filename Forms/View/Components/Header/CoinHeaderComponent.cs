...
namespace MyCC.Forms.View.Components.Header
{
    public class CoinHeaderComponent : HeaderView
    {
        ...
        private void UpdateView()
        {
            ...
            {
                TitleText = Sum.ToStringTwoDigits(ApplicationSettings.RoundMoney, Sum.Amount < 10000);
                CodeText = Sum.Amount < 10000 ? string.Empty : Sum.Currency.Code;
        ...
        private Money CoinSumAs(Currency c) => new Money(CoinSum.Amount * (RateHelper.GetRate(CoinSum.Currency, c)?.Rate ?? 0), c);
