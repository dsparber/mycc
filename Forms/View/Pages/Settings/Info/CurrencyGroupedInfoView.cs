...
namespace MyCC.Forms.View.Pages.Settings.Info
{
	public class CurrencyGroupedInfoView : ContentPage
	{
		...
		private EventHandler OpenFor(IEnumerable<Currency> currencies, string title) => (sender, e)
		=> Navigation.PushAsync(new CurrencyOverlay(new Func<IEnumerable<Currency>>(() => currencies), title, false, true));
